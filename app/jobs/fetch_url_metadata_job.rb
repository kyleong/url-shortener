class FetchUrlMetadataJob < ApplicationJob
  ...
  def perform(url_id)
    ...
    puts "Fetched metadata for URL ID #{url_id}: #{metadata.inspect}"
    if metadata
      url.update(fetch_status_code: metadata[:status_code], title: metadata[:title], fetched_at: Time.current)
    end
  end
